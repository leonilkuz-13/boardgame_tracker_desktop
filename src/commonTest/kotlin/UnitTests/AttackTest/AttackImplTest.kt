...
class AttackImplTest {
    ...
    fun `single attack on empty cell results in Miss`() {
        ...
        assertTrue(result is MoveResult.Success.Miss, "Expected result to be Miss")
        assertEquals(CellStatus.MISS, board.getCellStatus(coordinate))
    }
    ...
    fun `single attack on ship results in Hit`() {
        ...
        assertTrue(result is MoveResult.Success.Hit, "Expected result to be Hit")
        assertEquals(CellStatus.HIT, board.getCellStatus(coord1))
        assertEquals(CellStatus.SHIP, board.getCellStatus(coord2))
    }
    ...
    fun `single attack sinking the last ship results in Over`() {
        ...
        assertTrue(result is MoveResult.Success.Over, "Expected result to be Over")
        assertTrue(submarine.isSunk(), "Submarine should be sunk")
    }
    ...
    fun `attack on already hit cell returns Error InvalidMove`() {
        ...
        assertTrue(result is MoveResult.Error.InvalidMove, "Expected an Error.InvalidMove")
    }
    ...
    fun `bomber attack hits multiple cells and returns GrandResult`() {
        ...
        assertTrue(result is MoveResult.GrandResult, "Expected result to be GrandResult")

        val grandResult = result as MoveResult.GrandResult
        assertTrue(grandResult.results.isNotEmpty(), "Bomber attack results list should not be empty")
        assertEquals(CellStatus.MISS, board.getCellStatus(center))
    }
}