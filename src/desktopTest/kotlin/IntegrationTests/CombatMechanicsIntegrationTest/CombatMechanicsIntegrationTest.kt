package IntegrationTests.CombatMechanicsIntegrationTest
...
class CombatMechanicsIntegrationTest {
    ...
    fun `sinking a ship automatically fills surrounding cells with MISS`() {
        ...
        assertEquals(
            ...
            "Player 1 should keep the turn after a hit"
        )

        assertEquals(
            CellStatus.BORDER,
            board2.getCellStatus(Coordinate('A', 2)),
            "Halo should not be filled on just a Hit"
        )

        val sunkResult = game.move(Move.SingleAttack(Coordinate('C', 2)))

        assertTrue(
            sunkResult is MoveResult.Success.Over,
            "Second shot should be OVER (Sunk the last ship)"
        )

        assertEquals(
            CellStatus.MISS,
            board2.getCellStatus(Coordinate('A', 2)),
            "Halo cell A2 should be updated to MISS"
        )
        assertEquals(
            CellStatus.MISS,
            board2.getCellStatus(Coordinate('A', 1)),
            "Halo cell A1 should be updated to MISS"
        )
        assertEquals(
            CellStatus.MISS,
            board2.getCellStatus(Coordinate('D', 3)),
            "Halo cell D3 should be updated to MISS"
        )
        assertEquals(
            CellStatus.MISS,
            board2.getCellStatus(Coordinate('C', 1)),
            "Halo cell C1 should be updated to MISS"
        )
    }
}